...
cat *.ntds | grep Enabled | grep -v '\$' | grep ::: | rev | cut -d ' ' -f2,3,4,5,6,7,8,9 | rev > ntds.all
echo "`wc -l ntds.all` user hashes"

#NT Hashes for hashtopolis
cat ntds.all | cut -d : -f4 | sort -u > ntds.nt.hashtopolis
echo "`wc -l ntds.nt.hashtopolis` unique user hashes"
...
#Blank Password
cat ntds.all | grep -av '31d6cfe0d16ae931b73c59d7e0c089c0' > ntds.no.pass
echo "`wc -l ntds.no.pass` accounts with blank passwords"

cat ntds.all | grep -av '8846f7eaee8fb117ad06bdd830b7586c' > ntds.password.as.password
